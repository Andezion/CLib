...
int main(void)
{
    ...
    const size_t N = 300;
    double data[N][2]; size_t labels[N];
    for (size_t i = 0; i < N; i++)
    {
        size_t cls = i % out_dim;
        labels[i] = cls;
        double angle = cls * 2.0 * 3.14159265 / out_dim;
    ...
    {
        double loss = 0.0; size_t correct = 0;
        for (size_t i = 0; i < N; i++)
        {
            x->data[0] = data[i][0]; x->data[1] = data[i][1];
            dense_forward(l1, x, a); relu_inplace(a);
            ...
            free_float_array(&d_a); free_float_array(&d_x);

            size_t pred = 0; double best = probs->data[0];
            for (size_t k = 1; k < out_dim; k++) if (probs->data[k] > best) { best = probs->data[k]; pred = k; }
            if (pred == labels[i]) correct++;
        }
        if (epoch % 50 == 0) printf("epoch %d loss=%.4f acc=%.2f%%\n", epoch, loss / N, (double)correct * 100.0 / (double)N);
    }

    dense_free(&l1); dense_free(&l2);
    free_float_array(&x); free_float_array(&a); free_float_array(&logits); free_float_array(&probs); free_float_array(&grad_out);
    return 0;
}
