...
int main(void)
{
    ...
    for (int epoch = 0; epoch < 800; epoch++)
    {
        ...
        {
            for (size_t d = 0; d < in_dim; d++) x->data[d] = ((double)rand() / RAND_MAX - 0.5) * 2.0;

            dense_forward(e1, x, h1); tanh_inplace(h1);
            dense_forward(e2, h1, code); tanh_inplace(code);

            dense_forward(d1, code, h2); tanh_inplace(h2);
            ...
            free_float_array(&d_h2); free_float_array(&d_code); free_float_array(&d_h1); free_float_array(&d_x);
        }
    ...
    for (int t = 0; t < 5; t++)
    {
        ...
        dense_forward(d1, code, h2); tanh_inplace(h2);
        dense_forward(d2, h2, y);
        printf("orig:"); for (size_t d = 0; d < in_dim; d++) printf(" %.3f", x->data[d]);
